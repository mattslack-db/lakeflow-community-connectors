...
| `lookback_seconds` | integer | `posts`, `pages`, `media` | `0` | Lookback applied only at the lower bound of the cursor range to re-capture records edited during a prior window. Handles clock skew / DST edge cases. The stored cursor is never widened by this. **`cdc` tables only** — ignored for append-only `comments`, where re-reading would duplicate rows. |
